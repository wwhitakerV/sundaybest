...
| Control    | Statement                                   | Status      | Where                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ---------- | ------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PLATFORM-1 | "The app uses IPC mechanisms securely."     | **done**    | `src/core/security/deep-links/validate-deep-link.ts` checks every inbound URL against a Zod allowlist of routes and params; anything else resolves to `/`. Percent-encoding, double-encoding, dot-segments, backslashes, control characters and over-long URLs are rejected on a **normalised** path, so a comparison cannot be fooled by a spelling of a route. The raw URL is never returned or logged. `src/app/+native-intent.tsx` is delegation only. |
...
| PLATFORM-3 | "The app uses the user interface securely." | **planned** | `usePrivacyScreen()` in `src/core/security/screen/` is ready: it blocks screenshots and blurs the app-switcher snapshot via `enableAppSwitcherProtectionAsync`. Still planned because **no screen calls it** — nothing sensitive is on screen yet — and a pasteboard policy and keyboard-cache review are outstanding.                                                                                                                                     |
...
| Control      | Statement                                              | Status      | Where                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------ | ------------------------------------------------------ | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RESILIENCE-1 | "The app validates the integrity of the platform."     | **planned** | Two of three parts exist: App Attest (prompt 7) and freeRASP runtime integrity (`src/core/security/integrity/`), whose policy maps each signal to clear-session / disable-sensitive / report-only and never crashes the app. Still planned because the **server-side verification does not exist**, without which all of it is evidence nobody checks — and because it is not yet mounted at startup. |
| RESILIENCE-2 | "The app implements anti-tampering mechanisms."        | **planned** | The response now exists and is wired: a tampering signal clears the session, and the API client refuses `sensitive` requests with `kind: "integrity"` once integrity has disabled them. Same gap as RESILIENCE-1 — the server must verify assertions, enforce the monotonic counter, and rate-limit per key.                                                                                          |
